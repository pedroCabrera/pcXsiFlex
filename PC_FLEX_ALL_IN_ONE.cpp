...
enum IDs
{
	ID_IN_maxParticles = 0,
	ID_IN_maxDiffuseParticles = 1,
	ID_IN_maxNeighborsPerParticle = 2,

	ID_IN_solver = 3,
	ID_IN_numIterations = 4,
	ID_IN_gravity = 5,
	ID_IN_radius = 6,
	ID_IN_solidRestDistance = 7,
	ID_IN_fluidRestDistance = 8,
	ID_IN_dynamicFriction = 9,
	ID_IN_staticFriction = 10,
	ID_IN_particleFriction = 11,
	ID_IN_restitution = 12,
	ID_IN_adhesion = 13,
	ID_IN_sleepThreshold = 14,
	ID_IN_maxSpeed = 15,
	ID_IN_maxAcceleration = 16,
	ID_IN_shockPropagation = 17,
	ID_IN_dissipation = 18,
	ID_IN_damping = 19,
	ID_IN_wind = 20,
	ID_IN_drag = 21,
	ID_IN_lift = 22,
	ID_IN_fluid = 23,
	ID_IN_cohesion = 24,
	ID_IN_surfaceTension = 25,
	ID_IN_viscosity = 26,
	ID_IN_vorticityConfinement = 27,
	ID_IN_anisotropyScale = 28,
	ID_IN_anisotropyMin = 29,
	ID_IN_anisotropyMax = 30,
	ID_IN_smoothing = 31,
	ID_IN_solidPressure = 32,
	ID_IN_freeSurfaceDrag = 33,
	ID_IN_buoyancy = 34,
	ID_IN_diffuseThreshold = 35,
	ID_IN_diffuseBuoyancy = 36,
	ID_IN_diffuseDrag = 37,
	ID_IN_diffuseBallistic = 38,
	ID_IN_diffuseSortAxis = 39,
	ID_IN_diffuseLifetime = 40,
	ID_IN_plasticThreshold = 41,
	ID_IN_plasticCreep = 42,
	ID_IN_collisionDistance = 43,
	ID_IN_particleCollisionMargin = 44,
	ID_IN_shapeCollisionMargin = 45,
	ID_IN_planes = 46,
	ID_IN_numPlanes = 47,
	ID_IN_relaxationMode = 48,
	ID_IN_relaxationFactor = 49,
	ID_IN_substeps = 50,
	ID_IN_fps = 51,
...
CStatus RegisterPC_FLEX_ALL_IN(PluginRegistrar& in_reg)
{
	ICENodeDef nodeDef;
	nodeDef = Application().GetFactory().CreateICENodeDef(L"PC_FLEX_ALL_IN", L"PC_FLEX_ALL_IN");
	...
	st = nodeDef.AddInputPort(ID_IN_types, ID_G_100, siICENodeDataLong, siICENodeStructureArray, siICENodeContextSingleton, L"Types", L"Types", CValue(), CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
	st = nodeDef.AddInputPort(ID_IN_transforms, ID_G_100, siICENodeDataMatrix44, siICENodeStructureArray, siICENodeContextSingleton, L"transforms", L"transforms", CValue(), CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
	st = nodeDef.AddInputPort(ID_In_dynamics, ID_G_100, siICENodeDataBool, siICENodeStructureArray, siICENodeContextSingleton, L"dynamics", L"dynamics", CValue(), CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
	...
	st = nodeDef.AddInputPort(ID_IN_positions, ID_G_100, siICENodeDataVector4, siICENodeStructureArray, siICENodeContextSingleton, L"outpositions", L"outpositions", CValue(), CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
	st = nodeDef.AddInputPort(ID_IN_velocities, ID_G_100, siICENodeDataVector3, siICENodeStructureArray, siICENodeContextSingleton, L"outvelocities", L"outvelocities", CValue(), CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
...
SICALLBACK PC_FLEX_ALL_IN_BeginEvaluate(ICENodeContext& in_ctxt)
{
	...
	CDataArray2DLong typesData(in_ctxt, ID_IN_types);
	CDataArray2DMatrix4f transformsData(in_ctxt, ID_IN_transforms);
	CDataArray2DBool dynamicsData(in_ctxt, ID_In_dynamics);
	...
	CDataArray2DMatrix4f::Accessor transforms = transformsData[0];
	CDataArray2DBool::Accessor dynamics = dynamicsData[0];
	CDataArray2DLong::Accessor types = typesData[0];

	for (int x = 0; x < transforms.GetCount(); ++x)
	{
		int dInd, tInd;
		dInd = (x <= dynamics.GetCount()) ? x : dynamics.GetCount() - 1;
		tInd = (x <= types.GetCount()) ? x : types.GetCount() - 1;
		TransformToCollideShape(FlexSys, transforms[x], transforms[x], dynamics[dInd], types[tInd]);
	}
	...
	int max = (positionsIce.GetCount() < FlexSys->maxParticles) ? positionsIce.GetCount() : FlexSys->maxParticles;
	FlexSys->g_buffers->activeIndices.resize(max);
	for (int x = 0; x < max; ++x)
	{
		int vInd = x;
		if (x >= velocitiesIce.GetCount()) vInd = velocitiesIce.GetCount() - 1;
		Vec3 pos, vel;
		...
		FlexSys->g_buffers->positions[x] = Vec4(pos, positionsIce[x].GetW());
		FlexSys->g_buffers->velocities[x] = vel;
		FlexSys->g_buffers->phases[x] = phase;
		FlexSys->g_buffers->activeIndices[x]=x;
		/*
		if (FlexSys->g_buffers->activeIndices.size() < FlexSys->maxParticles)
			FlexSys->g_buffers->activeIndices.push_back(FlexSys->cursor);
		if (FlexSys->cursor<FlexSys->maxParticles - 1)
			FlexSys->cursor++;
		else
			FlexSys->cursor = 0;
			*/
...
SICALLBACK PC_FLEX_ALL_IN_Evaluate(ICENodeContext& in_ctxt)
{
	...
	case ID_OUT_positions:
	{
		...
		if (!userData.IsEmpty()) {
			...
			{
				outPositions[x].Set(FlexSys->g_buffers->positions[x].x, FlexSys->g_buffers->positions[x].y, FlexSys->g_buffers->positions[x].z, FlexSys->g_buffers->positions[x].w);
			}
	...
	case ID_OUT_velocities:
	{
		...
		if (!userData.IsEmpty()) {
			...
			for (int x = 0; x < FlexSys->g_buffers->activeIndices.size(); ++x)
			{
				outvelocities[x].Set(FlexSys->g_buffers->velocities[x].x, FlexSys->g_buffers->velocities[x].y, FlexSys->g_buffers->velocities[x].z);
			}
			//FlexSys->g_buffers->UnmapBuffers();
		}
	}
...
/*
SICALLBACK PC_FLEX_SOLVER_EndEvaluate(ICENodeContext& in_ctxt)
{
Application().LogMessage(CString("TERM"));
...
*/
SICALLBACK PC_FLEX_ALL_IN_Term(ICENodeContext& in_ctxt)
{
